...
//	public function  __construct($phpbb_root_path, \phpbb\cache\driver\driver_interface $cache = null ) 
//	{
	//	$this->finder = $finder;
	//	$this->cache = $cache;
	//	phpbb\hook\finder
...
//	}
	...
	//	$user->add_lang('faq_manager');
		$user->add_lang_ext('hammer663/FaqManager', 'faq_manager');
		...
		$template->assign_vars(array(
			...
			'S_UNDO'			=> ($file && file_exists($phpbb_root_path . 'ext/hammer663/FaqManager/store/faq_backup/' . $file . '.' . $phpEx . '.bak')) ? true : false,
		...
		else
		{
			...
			//if ($rules_board)
			foreach ($this->get_rules_file_list() as $loc)
			{
				$template->assign_block_vars('file_list', array(
					'NAME'			=> substr($loc, (strpos($loc, '/help_') + 6)),
					'LANGUAGE'		=> substr($loc, 0, strpos($loc, '/')),
					'LOCATION'		=> 'ext/hammer663/BoardRules/language/' . $loc . '.' . $phpEx,
		...
		{
			case 'undo' :
				@copy($phpbb_root_path . 'ext/hammer663/FaqManager/store/faq_backup/' . $file . '.' . $phpEx . '.bak', $phpbb_root_path . 'language/' . $file . '.' . $phpEx);
...
?>';
		...
		if (!@is_writable($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx))
		{
			@chmod($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx, 0777);
		}
...
//		if (!@is_writable($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx))
//		{
//			trigger_error($this->user->lang['FAQ_FILE_NO_WRITE'] . ' - ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx);			
//		}
//		else if (!@is_writable($phpbb_root_path . 'language/' . $file . '.' . $phpEx))
//		{
...
//		}
		...
		if ($fp = @fopen($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx, 'wb'))
		{
			@flock($fp, LOCK_EX);
			@fwrite ($fp, $output);
			@flock($fp, LOCK_UN);
	...
	*/
	...
	function get_rules_file_list()
	{
		global $phpbb_root_path, $phpEx;

		$list = array();
		$dh = @opendir($phpbb_root_path . 'ext/hammer663/BoardRules/language/');

		if ($dh)
		{
			while (($file = readdir($dh)) !== false)
			{
				if (strpos($file, '.') === false && is_dir($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file))
				{
					$dh1 = @opendir($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file);
					if ($dh1)
					{
						while (($file1 = readdir($dh1)) !== false)
						{
							...
							{
								if (strpos($file1, '.') === false && is_dir($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '/' . $file1))
								{

									$dh2 = @opendir($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '/' . $file1);
									if ($dh2)
									{
										while (($file2 = readdir($dh2)) !== false)
										{
	...
	*/
	function load_faq($file)
	{
		...
		else if (file_exists($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx))
		{
			include($phpbb_root_path . 'ext/hammer663/BoardRules/language/' . $file . '.' . $phpEx);
			if (!isset($help) || !is_array($help))
			{
				trigger_error('BAD_FAQ_FILE');
			}
